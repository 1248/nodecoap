        ...
        listeners.push({
            handler: function(rsppkt, raw) {
                ...
                if (rsppkt.getHeaderType() == erbium.COAP_TYPE_CON) {
                    ...
                    if (opts.beforeSend !== undefined) {
                        opts.beforeSend(new Buffer(ackraw));    // copy of
                    }
                ...
                for (var i=0;i<app.listeners.length;i++) {
                    if (inpkt.getHeaderMID() == app.listeners[i].id && 0 === app.listeners[i].tok.compare(inpkt.getHeaderToken())) {
                        app.listeners[i].handler(inpkt, msg);
                        app.listeners.splice(i, 1);
                        break;
                    }
                }
                */
