...
function handle(endpoints, ackwaiters, inpkt, sendfunc) {
    ...
    if (undefined === ep || undefined === method) {
        var outpkt = new erbium.Erbium(type, erbium.NOT_FOUND_4_04, inpkt.getHeaderMID());
        outpkt.setHeaderToken(inpkt.getHeaderToken());
        sendfunc(outpkt);
    } else
    if (undefined === (f = ep[req.method])) {
        var outpkt = new erbium.Erbium(type, erbium.METHOD_NOT_ALLOWED_4_05, inpkt.getHeaderMID());
        outpkt.setHeaderToken(inpkt.getHeaderToken());
        sendfunc(outpkt);
    }
    else {
        var outpkt = new erbium.Erbium(type, erbium.CONTENT_2_05, inpkt.getHeaderMID());
        var res = {
