...
function receive(msg, rinfo) {
    ...
    coap.build(outpkt);
    //console.log(outpkt);
    console.log(outpkt.raw, 0, outpkt.raw.length, rinfo.port, rinfo.address);
    FIXME does client need to fill out a port option or something?
    this.bearer.send(outpkt.raw, 0, outpkt.raw.length, 5683, rinfo.address, function() {
        console.log("server sent rsp");
    });
}
